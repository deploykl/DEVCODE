...
const LISTAR = async () => {
  try {
    ...
    const responseGrupo = await api.get('poi/grupo/',{ headers: { Authorization: `Bearer ${token}` } });
...
const ADD = async () => {
  try {
    ...
    const response = await api.post('poi/grupo/', form.value, { headers: { Authorization: `Bearer ${token}` } });
...
const UPDATE_ID = (grupo) => {
  ...
  Object.assign(form.value, grupo); // Copiar directamente los valores al form
...
const UPDATE = async () => {
  try {
    ...
    const token = getAuthToken();
    const response = await api.put(`poi/grupo/${id}/`, {
      ...rest,
      Grupo,
    }, {
    ...
    const index = Grupo.value.findIndex(item => item.id === id);
